...
public class PA007isFibonacciHuge {
	...
	public static long getFibonacciHuge(long n, long m) {
		...
		if (l < 0)
			...
			np = n % l;
		BigInteger fib = calc_fib_huge(np);
		BigInteger rm = fib.remainder(BigInteger.valueOf(m));
		return rm.longValue();
	}
	...
	public static BigInteger calc_fib_huge(long n) {
		...
		for (BigInteger i = BigInteger.valueOf(2); i.compareTo(bn) <= 0; i = i.add(BigInteger.valueOf(1))) {
